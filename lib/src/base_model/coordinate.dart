...
Direction opposite(Direction fromDir) =>
    DIRECTIONS[DIRECTIONS.indexOf(fromDir) + 3 % DIRECTIONS.length];
Direction clockwise(Direction fromDir) =>
    DIRECTIONS[DIRECTIONS.indexOf(fromDir) + 1 % DIRECTIONS.length];
Direction counterClockwise(Direction fromDir) =>
    DIRECTIONS[DIRECTIONS.indexOf(fromDir) - 1 % DIRECTIONS.length];
...
class Coordinate {
  ...
  static Coordinate fromKey(int key) => _coordCache.putIfAbsent(
      key, () => new Coordinate(_coordHKey(key), _coordVKey(key)));
  ...
  static Map<Direction, int> getNeighbors(int key) =>
      _getNeighbors(_coordHKey(key), _coordVKey(key));
  static Map<Direction, int> _getNeighbors(int h, int v) {
    ...
    neighborMap[Direction.SouthWest] =
        Coordinate._getKey(h - ((v + 1) % 2), v + 1);
    neighborMap[Direction.West] = Coordinate._getKey(h - 1, v);
    neighborMap[Direction.NorthWest] =
        Coordinate._getKey(h - ((v + 1) % 2), v - 1);
    return neighborMap;
  }

  static Point getPoint(int key) =>
      validKey(key) ? _getPoint(_coordHKey(key), _coordVKey(key)) : null;
  ...
  static CoordinateType getType(int key) =>
      _getType(_coordHKey(key), _coordVKey(key));
  static CoordinateType _getType(int h, int v) =>
      (h - (v % 2)) % 3 == 1 ? CoordinateType.Tile : CoordinateType.Plot;
  ...
  Map<Direction, int> neighbors({CoordinateType ofType}) {
    ...
    DIRECTIONS.forEach((dir) {
      ...
      if (ofType == null || Coordinate.getType(key) == ofType)
        keyMap[dir] = key;
    });
    return keyMap;
  }
