...
class Edge {
  ...
  static Edge fromKey(int key) => _edgeCache.putIfAbsent(
      key, () => new Edge(_edgeAKey(key), _edgeBKey(key)));
  ...
  static int getKey(int aKey, int bKey) =>
      max(aKey, bKey) * 10000 + min(aKey, bKey);
  ...
  double get length =>
      Coordinate.getPoint(_aKey).distanceTo(Coordinate.getPoint(_bKey));
  ...
  String toString() =>
      "E${key}{${Coordinate.fromKey(_aKey).point} <-> ${Coordinate.fromKey(_bKey).point}}";
}
