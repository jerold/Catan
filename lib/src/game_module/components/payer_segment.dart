...
class _PayerSegment extends w_flux.FluxComponent<GameActions, TradePayload> {
  ...
  render() {
    ...
    COMMODITIES.forEach((commodity) {
      ...
      if (payer != null) {
        if (commodity == Commodity.Unknown) {
          ...
          if (payer.commodities[commodity] <= 0)
            classes = '${classes} secondary inverted disabled';
          else
            classes = '${classes} grey';
        }
      } else {
    ...
    COMMODITIES.forEach((commodity) {
      labels.add(react.div({'className': 'center aligned column'},
          '${stringFromCommodity(commodity)}'));
    });

    List deposits = new List();
    COMMODITIES.forEach((commodity) {
      bool disabled = !trade.exchange.containsKey(commodity) ||
          trade.exchange[commodity] <= 0;
      int count = trade.exchange[commodity] ?? 0;
      deposits.add(react.div(
          {'className': 'column'},
          react.button({
            'className':
                'ui ${disabled ? "secondary inverted disabled" : "white"} button',
    ...
    if (trade.requiresSatisfaction()) {
      headerItems.add(react.div({
        'className':
            'ui ${trade.canComplete() ? "green" : "red"} top right attached label'
